...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
    ...
    char *noa_my_love="_<3KIREL";
	char *temp = (char*)malloc((strlen(filedata->string_table+section->sh_name)+strlen(noa_my_love)+1)*sizeof(char));
	temp[0]='\0';
	strcat(temp,filedata->string_table+ section->sh_name);
	strcat(temp,noa_my_love);
	//print_symbol (-17, SECTION_NAME_PRINT (section));
	print_symbol (-17, temp);
	free(temp);
	}
	else{
		print_symbol (-17, SECTION_NAME_PRINT (section));
...
process_symbol_table (Filedata * filedata)
{
  ...
  if ((filedata->dynamic_info[DT_HASH] || filedata->dynamic_info_DT_GNU_HASH)
	   ...
	   i++, section++)
	{
	  ...
	  {
	    if (ELF_ST_BIND ((symtab+si)->st_info)==STB_GLOBAL)
		global_count++;
